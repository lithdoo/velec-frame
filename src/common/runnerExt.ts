...
export interface SqliteRunnerStep
  extends RunnerTaskStep<{
    fileUrl: string
    sql: string
    useParams: boolean
    type: 'run' | 'query'
...
export interface JsonDataRunnerStep
  extends RunnerTaskStep<{
    receiveId: string
    default: any
  }> {
...
export interface ScopeDataRunnerStep
  extends RunnerTaskStep<{
    fields: string[]
  }> {
...
export const isScopeDataRunnerStep = (
  step: RunnerTaskStep<unknown>
): step is ScopeDataRunnerStep => {
  return step.worker === 'scope-data-runner'
}

export interface NodeJsRunnerStep
  extends RunnerTaskStep<{
    code: string
    workdir: string
  }> {
