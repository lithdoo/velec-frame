...
export class ModalStackHandlerBase {
  stacks: ModalPanel[] = []
  push(content: ModalPanel) {
...
const props = defineProps<{ handler: ModalStackHandlerBase }>()
...
<template>
  <div class="confirm-modal-cntr" v-if="current">
    <div class="confirm-modal__overlay" v-if="current"></div>
    <div class="confirm-modal" v-if="current">
      ...
      <!-- </div> -->
      <!-- <div
        class="confirm-modal__extra"
        v-for="(_, i) in new Array(extraLenth).fill(0)"
        :key="i"
      ></div> -->
    </div>
  </div>
