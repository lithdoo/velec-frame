...
export type MutDomHTMLElementOption<
  Input extends {
    className: any
    style: any
    attrStyle: any
...
> {
  ...
  protected addNodeDisposeListeners(node: Node, call: () => void) {
    this.nodeDisposeListeners.set(
      node,
      (this.nodeDisposeListeners.get(node) ?? []).filter((c) => c !== call).concat([call])
    )
  }
  ...
  htmlElement<T extends HTMLElement>(node: T, option: MutDomHTMLElementOption<Input>) {
    ...
    if (this.handler.isMutValue(cls)) {
      this.addNodeDisposeListeners(
        ...
        this.handler.onValueChange(
          ...
          () => {
            ...
            node.className = (typeof rawContent === 'string' ? [rawContent] : rawContent)
              .filter((v) => !!v)
              .join(' ')
          },
          true
      ...
      node.className = (typeof cls === 'string' ? [cls] : (cls as string[]))
        .filter((v) => !!v)
        .join(' ')
    }
