import {
  ...
  MBaseCondRenderNode,
  mbaseDomRedner,
  MBaseElementRenderNode,
  MBaseGroupFragment,
  MBaseLoopRenderNode,
  MBaseParentRenderNode,
  MBaseRenderFragment,
  MBaseTextRenderNode,
  MBaseWacher
} from './mut'
...
export type Ext<Current extends {}, Next extends {}> = {
  [Key in keyof (Next & Current)]: Next extends { [K in Key]: any }
    ? Next[Key]
    : Current extends { [K in Key]: any }
      ? Current[Key]
      : never
}
...
export interface MBaseTempalteParentNode<
  Scope extends Record<string, unknown>,
  ChildScope extends Record<string, unknown> = Scope
> extends MBaseTempalteNode<Scope> {
  children: MBaseTempalteNode<ChildScope>[]
}

export interface MBaseElementTemplateNode<
  T extends HTMLElement,
  Scope extends Record<string, unknown>
> extends MBaseTempalteParentNode<Scope> {
...
export function isMBaseElementTempalteNode<
  T extends HTMLElement,
  Scope extends Record<string, unknown>
>(ele: Record<string, any>): ele is MBaseElementTemplateNode<T, Scope> {
  return ele['_type'] === MBaseTempalteNodeType.Element
}

export interface MBaseTextTemplateNode<Scope extends Record<string, unknown>>
  extends MBaseTempalteNode<Scope> {
  _type: MBaseTempalteNodeType.Text
  text: (scope: MBaseTScope<Scope>) => MayBeMBase<string>
}

export function isMBaseTextTempalteNode<Scope extends Record<string, unknown>>(
  ele: Record<string, any>
): ele is MBaseTextTemplateNode<Scope> {
  return ele['_type'] === MBaseTempalteNodeType.Text
}

export interface MBaseCondTemplateNode<Scope extends Record<string, unknown>>
  extends MBaseTempalteParentNode<Scope> {
...
export function isMBaseCondTempalteNode<Scope extends Record<string, unknown>>(
  ele: Record<string, any>
): ele is MBaseCondTemplateNode<Scope> {
  return ele['_type'] === MBaseTempalteNodeType.Cond
}

export interface MBaseLoopTemplateNode<T, Scope extends Record<string, unknown>>
  extends MBaseTempalteParentNode<Scope, Ext<Scope, { _index: number; _item: T }>> {
  _type: MBaseTempalteNodeType.Loop
  list: (scope: MBaseTScope<Scope>) => MayBeMBase<T[]>
}

export function isMBaseLoopTempalteNode<T, Scope extends Record<string, unknown>>(
  ele: Record<string, any>
): ele is MBaseLoopTemplateNode<T, Scope> {
  return ele['_type'] === MBaseTempalteNodeType.Loop
}

export interface MBasePropTemplateNode<
  Scope extends Record<string, unknown>,
  Next extends Record<string, unknown>
> extends MBaseTempalteParentNode<Scope, Next> {
  _type: MBaseTempalteNodeType.Prop
  trans: (scope: MBaseTScope<Scope>) => Next
}

export function isMBasePropsTempalteNode<
  Scope extends Record<string, unknown>,
  Next extends Record<string, unknown>
>(ele: Record<string, any>): ele is MBasePropTemplateNode<Scope, Next> {
...
export function render<Scope extends Record<string, unknown>>(
  template: MBaseTempalteNode<Scope>,
  ...
  if (isMBaseTextTempalteNode<Scope>(template)) {
    ...
    // todo
    const renderNode = new MBaseLoopRenderNode(
      list,
      (_item, _index) => {
        ...
        return new MBaseGroupFragment(
          template.children.map((template) => render(template, childScope))
        )
      },
...
export function renderChildren<Scope extends Record<string, unknown>>(
  templates: MBaseTempalteNode<Scope>[],
  scope: MBaseTScope<Scope>
): MBaseGroupFragment {
