...
import { JthTemplate, JthTemplateBase, JthTemplateGroup, JthTemplateRoot, JthTemplateType, ValueGeneratorRef } from "./JthTemplate"
...
export class JthState {
    ...
    updateNode<T extends JthTemplate>(
        id: string,
...
export class JthStateController {
    static staticValueRef(input: 'null' | 'blank_string' | 'empty_array' | 'empty_object' | 'true' | 'false'): ValueGeneratorRef {
        return { ["_VALUE_GENERATOR_REFERENCE_"]: input }
    }

    static staticValue(input: 'null' | 'blank_string' | 'empty_array' | 'empty_object' | 'true' | 'false'): ValueGenerator {
        switch (input) {
            case 'null':
