...
export class DBChartService extends DBService {
  ...
  async updateRenderRecords(records: RenderStoreRecord[]) {
    const sql = `INSERT OR REPLACE INTO ${HiddenTable.Render} (table_name, pos_left, pos_top, size_width, size_height, color, list_idx ) VALUES ${records
      .map(
        (record) => `
                ('${record.table_name}', ${record.pos_left}, ${record.pos_top}, ${record.size_width}, ${record.size_height}, '${record.color}', ${record.list_idx})`
      )
      .join(',')}`
    await this.run(sql)
  }
  ...
  async clearData() {
    const sqls = Array.from(Object.values(HiddenTable)).flatMap((table) => [`DROP TABLE ${table};`])
  ...
  async sortTable(list: { table: string; idx: number }[]) {
    const sqls = list.map(
      ({ table, idx }) =>
        `UPDATE ${HiddenTable.Render} SET list_idx = ${idx} WHERE table_name = '${table}'`
    )
  ...
  async deleteTable(name: string) {
    const sqls = [
      `DROP TABLE ${name};`,
      `DELETE FROM ${HiddenTable.Render} WHERE table_name = '${name}'`
    ]
    await this.runList(sqls)
  }
  ...
  async renameField(table: SqliteTableInfo, oldName: string, newName: string) {
    ...
    const createNewTempTable = `CREATE TABLE IF NOT EXISTS ${table.name}_temp (${table.fieldList
      .map((col, idx) => {
        return `${idx !== 0 ? ',\n' : '\n'}   ${col.name === oldName ? newName : col.name} ${
          col.type
        } ${col.primaryKey ? 'PRIMARY KEY' : ''} ${col.notNull ? 'NOT NULL' : ''}`
      })
      .join('')}\n);`
    ...
    await this.runList([createNewTempTable, moveData, dropOldTable, renameNewTable])
  }
  ...
  async deleteField(table: SqliteTableInfo, field: string) {
    ...
    const createNewTempTable = `CREATE TABLE IF NOT EXISTS ${table.name}_temp (${fields
      .map((col, idx) => {
        return `${idx !== 0 ? ',\n' : '\n'}   ${col.name} ${col.type} ${
...
export class DBRecordsService extends DBService {
  ...
  async insertToTable(
    ...
    type: 'INSERT' | 'REPLACE' | 'INSERT OR REPLACE' = 'INSERT'
  ) {
    const sql = `
