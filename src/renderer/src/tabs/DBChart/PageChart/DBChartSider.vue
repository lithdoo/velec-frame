  ...
  <div class="db-chart-sider">
    ...
    <div class="db-chart-sider__body">
      <template v-for="(entity, index) in entities" :key="entity.id">
        <div
          class="db-chart-sider__divide-before"
          :data-highlight="entitySorter.insertPos() === index"
        ></div>
        <div class="db-chart-sider__entity-item" :data-draging="entitySorter.current === entity.id">
          <div
            class="db-chart-sider__drag-move-before"
            @drop="(e) => entitySorter.drop(e)"
            @dragover="(e) => entitySorter.over(e, 'before', entity.id)"
            v-show="entitySorter.current"
          ></div>
          <div
            class="db-chart-sider__drag-move-after"
            @drop="(e) => entitySorter.drop(e)"
            @dragover="(e) => entitySorter.over(e, 'after', entity.id)"
            v-show="entitySorter.current"
          ></div>

          <div
            class="db-chart-sider__entity-color"
            :style="{ backgroundColor: entity.render.color }"
          ></div>
          <div
            class="db-chart-sider__entity-item-header"
            @click="() => toggleEntity(entity.table.name)"
          >
            <div
              class="db-chart-sider__dragger"
              draggable="true"
            ...
            <template
              v-if="
                entityNameEditor.current && entityNameEditor.current.oldName === entity.table.name
              "
            >
              <VxInput
                class="db-chart-sider__entity-name-editor"
                @click.native.stop
                v-model="entityNameEditor.current.newName"
                :placeholder="'请输入新表名'"
              >
                <template #suffix>
              ...
              <div class="db-chart-sider__entity-fields">
                <div class="db-chart-sider__field-title-with-creator">
                  <div class="db-chart-sider__field-title">
                    ...
                    <div class="db-chart-sider__field-title-text">Fields</div>
                    <div
                      class="db-chart-sider__field-title-btns"
                      @click.stop
                    ...
                    >
                      ...
                      <template v-else>
                        <VxButton only-icon icon="plus" :click="() => fieldCreator.start(entity)">
                        </VxButton>
                      </template>
                    </div>
                  </div>
                  <div
                    class="db-chart-sider__field-creator"
                    v-if="fieldCreator.current && fieldCreator.current.entity === entity"
                  >
                    <div class="db-chart-sider__field-creator-name">
                      <VxInput
                        class="db-chart-sider__field-name-editor"
                        @click.native.stop
                        v-model="fieldCreator.current.name"
                        :placeholder="'请输入新字段名'"
                      >
                      </VxInput>
                    </div>
                    <div class="db-chart-sider__field-creator-type">
                      <VxSelector
                        :options="fieldCreator.options"
                        v-model="fieldCreator.current.type"
                      ></VxSelector>
                    </div>
                  </div>
                </div>
                <div
                  class="db-chart-sider__field-item"
                  v-for="field in entity.table.fieldList"
                  :key="field.name"
                >
                  <template
                    v-if="
                      fieldNameEditor.current &&
                      fieldNameEditor.current.oldName === field.name &&
                      fieldNameEditor.current.entity === entity
                    "
                  >
                    <VxInput
                      class="db-chart-sider__field-name-editor"
                      @click.native.stop
                      v-model="fieldNameEditor.current.newName"
                      :placeholder="'请输入新字段名'"
                    >
                      <template #suffix>
                        <VxButton only-icon icon="clear" :click="() => fieldNameEditor.cancel()">
                        </VxButton>
                  ...
                  <template v-else>
                    <div class="db-chart-sider__field-item-name" :title="field.name">
                      {{ field.name }}
                    </div>
                    <div class="db-chart-sider__field-item-type">[{{ field.type }}]</div>

                    <div
                      class="db-chart-sider__field-item-btns"
                      @click.stop
                      :title="entity.table.name"
                    >
                      ...
                      <VxButton
                        only-icon
                        icon="del"
                        v-if="entity.table.fieldList.length > 1"
        ...
        <div
          class="db-chart-sider__divide-after"
          :data-highlight="entitySorter.insertPos() - 1 === index"
        ></div>
      </template>
    </div>
  </div>
...
const entities = computed(() => {
  ...
  const list = state.value.nodeIds
    .map((id) => state.value.nodes.get(id))
    .filter((v) => !!v) as EntityData[]
...
const entitySorter = fixReactive(
  new (class {
...
const entityNameEditor = fixReactive(
  new (class {
...
const fieldNameEditor = fixReactive(
  new (class {
    current: {
      oldName: string
      newName: string
...
const fieldCreator = fixReactive(
  new (class {
    current: {
      name: string
      type: {
