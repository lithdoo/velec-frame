<template>
    <div class="mv-template">
        <div class="mv-template__sub-title">Test</div>
        <div class="mv-template__component">
            <EditValue :valRef="template.test" :controller="controller" :onchange="onChange"></EditValue>
        </div>
    </div>
</template>
...
import { MVTemplateCond } from '@renderer/mods/mutv-template';

const props = defineProps<{
    controller: MVFileController,
    template: MVTemplateCond
}>()

const onChange = (test:EvalRef)=>{
    props.controller.template.updateNode<MVTemplateCond>(props.template.id,(node)=>{
        return { ...node , test }
    })
}
